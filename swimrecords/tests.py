...
class SwimRecordTestCase(TestCase):
    ...
    def test_04_validate_relay_presence(self):
        ...
        try:
            ...
            self.assertTrue("'None' value must be either True or False." not in e.message_dict['relay'])
    ...
    def test_08_no_break_record_before_set_record(self):
        ...
        record = SwimRecord(first_name='j', last_name='j', team_name='k', relay=True, stroke='butterfly', distance=100,
                            record_date=timezone.now(), record_broken_date=(timezone.now() - timedelta(days=1)))
        record.save()
        try:
            record.full_clean()
            raise Exception("full_clean should throw a validation error")
